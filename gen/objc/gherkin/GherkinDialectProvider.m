  ...
  if (self == [GherkinGherkinDialectProvider class]) {
    {
      @try {
        ...
        for (jint len; (len = [dialects readWithCharArray:buffer]) > 0; ) {
          [builder appendWithCharArray:buffer withInt:0 withInt:len];
        }
        OrgJsonJSONObject *obj = create_OrgJsonJSONObject_initWithNSString_([builder description]);
        JreStrongAssignAndConsume(&GherkinGherkinDialectProvider_DIALECTS, new_JavaUtilHashMap_init());
        id<JavaUtilIterator> keys = [obj keys];
        while ([((id<JavaUtilIterator>) nil_chk(keys)) hasNext]) {
          NSString *key = [keys next];
          OrgJsonJSONObject *node = [obj optJSONObjectWithNSString:key];
          id<JavaUtilMap> elements = create_JavaUtilHashMap_init();
          id<JavaUtilSet> innerKeys = [((OrgJsonJSONObject *) nil_chk(node)) keySet];
          for (NSString * __strong k in nil_chk(innerKeys)) {
            OrgJsonJSONArray *values = [node getJSONArrayWithNSString:k];
            id<JavaUtilList> entries = create_JavaUtilArrayList_init();
            for (jint i = 0; i < [((OrgJsonJSONArray *) nil_chk(values)) length]; i++) {
              [entries addWithId:[values getStringWithInt:i]];
            }
            [elements putWithId:k withId:entries];
          }
          [((id<JavaUtilMap>) nil_chk(GherkinGherkinDialectProvider_DIALECTS)) putWithId:key withId:elements];
        }
      }
      ...
      @catch (OrgJsonJSONException *je) {
        @throw create_JavaLangRuntimeException_initWithJavaLangThrowable_(je);
      }
